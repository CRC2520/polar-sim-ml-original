...
import time
import numpy as np
from r8_completion.io import atomic_json, atomic_npz, _atomic_bytes, sha256, write_manifest
...
def save_rollout(directory, result, files):
    ...
    if events:
        ...
        # Lossless deterministic storage; compression does not consume an RNG.
        payload = gzip.compress(content.encode('utf-8'), compresslevel=6, mtime=0)
        files.append(_atomic_bytes(directory/'workspace_events.jsonl.gz', payload))
