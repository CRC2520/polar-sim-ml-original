...
def main():
    ...
    if freeze.get("status")!="D3_FROZEN_INITIALIZATION_QUALIFIED_NOT_EXECUTED":raise RuntimeError("D3 init freeze not qualified")
    rows=[]
    for block in range(d3.BLOCKS):
        ...
        for condition in d3.CONDITIONS:
            ...
            expected_support=torch.from_numpy(__import__('b1s.execution.core',fromlist=['matrix']).matrix(cfg["mask"]))
