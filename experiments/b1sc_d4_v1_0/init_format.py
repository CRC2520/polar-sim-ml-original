"""Canonical trainable-only byte format for B1-SC-D4 initialization snapshots."""
...
def decode_snapshot(data: bytes):
    if not data.startswith(MAGIC):
        raise RuntimeError("Wrong D4 snapshot magic")
    pos = len(MAGIC)
    n = struct.unpack(">Q", data[pos:pos+8])[0]
    pos += 8
    header = json.loads(data[pos:pos+n].decode("utf-8"))
    pos += n
    if (
        header.get("schema") != SCHEMA
        or header.get("trainable_only") is not True
        or header.get("support_buffer_included") is not False
        or header.get("temporal_gate_state_included") is not False
        or header.get("condition_included") is not False
    ):
        raise RuntimeError("Wrong D4 snapshot contract")
    payload = memoryview(data)[pos:]
    states = {"actor": {}, "critic": {}}
    for e in header["tensors"]:
        start = int(e["offset"])
        end = start + int(e["nbytes"])
        arr = np.frombuffer(payload[start:end], dtype=np.dtype(e["dtype"])).reshape(e["shape"]).copy()
        prefix, name = e["name"].split(".", 1)
        states[prefix][name] = arr
    return header, states


def write_snapshot(path, block, source_seed, actor, critic):
    data = encode_snapshot(block, source_seed, actor, critic)
...
def load_trainable_into(path, actor, critic, expected_sha256: str | None = None):
    ...
    if expected_sha256 is not None and actual != expected_sha256:
        raise RuntimeError("Frozen D4 init SHA-256 mismatch")
    header, states = decode_snapshot(data)
    for key, module in (("actor", actor), ("critic", critic)):
        params = dict(module.named_parameters())
        if set(params) != set(states[key]):
            raise RuntimeError(f"D4 trainable parameter set mismatch: {key}")
        with torch.no_grad():
            for name, p in params.items():
                src = torch.from_numpy(states[key][name]).to(dtype=p.dtype, device=p.device)
                if tuple(src.shape) != tuple(p.shape):
                    raise RuntimeError(f"D4 trainable shape mismatch: {key}.{name}")
                p.copy_(src)
