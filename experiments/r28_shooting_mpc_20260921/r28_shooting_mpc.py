...
def shooting_action(A,B,Q,R,x,uclip,horizon=24):
    K=base.infinite_lqr_gain(A,B,Q,R)
    if K is None:
        return 0.0
    center=float(-(K@x)[0])
    # First-action shooting with an LQR tail. The grid includes the LQR action.
    cand=np.unique(np.r_[
        np.linspace(-uclip,uclip,41),
        np.clip(center+np.linspace(-0.20*uclip,0.20*uclip,17),-uclip,uclip),
        np.array([np.clip(center,-uclip,uclip)])
    ])
    best=float(np.clip(center,-uclip,uclip)); bestv=float("inf")
    for u0 in cand:
        xx=A@x+B[:,0]*u0
        val=float(x@Q@x+R[0,0]*u0*u0)
        for _ in range(horizon-1):
            uu=float(np.clip(-(K@xx)[0],-uclip,uclip))
            val+=float(xx@Q@xx+R[0,0]*uu*uu)
            xx=A@xx+B[:,0]*uu
            if not np.all(np.isfinite(xx)) or np.linalg.norm(xx)>1e5:
                val=float("inf"); break
        if val<bestv:
...
def run_task(seed,task,controller,steps):
    ...
    for t in range(steps):
        ...
        if len(actions)>=80 and t%20==0 and controller in ("CORE","GENERIC","SHOOTING_MPC"):
            w=min(320,len(actions)); seq=np.asarray(states[-w:]+[est.copy()],float); act=np.asarray(actions[-w:],float)
            cut=max(50,int(.75*w))
            try:
        ...
        if controller=="ORACLE_MPC":
            ...
            u=shooting_action(At,Bt,Q,R,state,uclip,24)
        elif controller=="FROZEN":
            u=float(-(K_nom@est)[0]) if K_nom is not None else 0.0
        elif controller=="SHOOTING_MPC":
            u=shooting_action(Ahat,Bhat,Q,R,est,uclip,24)
        else:
        ...
        if rid!=0 and (t%seg)<80: shift_costs.append(c)
...
def eval_seed(seed,steps):
    ctrls=["CORE","GENERIC","SHOOTING_MPC","FROZEN","ORACLE_MPC"]
    tasks={}
    ...
    return {"seed":seed,"tasks":tasks,
            "core_mpc_cost_ratio":ratio("CORE","SHOOTING_MPC","mean_cost"),
            "core_mpc_postshift_ratio":ratio("CORE","SHOOTING_MPC","postshift_cost"),
            "core_generic_cost_ratio":ratio("CORE","GENERIC","mean_cost"),
            "mpc_frozen_ratio":ratio("SHOOTING_MPC","FROZEN","mean_cost"),
            "core_oracle_ratio":ratio("CORE","ORACLE_MPC","mean_cost"),
            "core_stable":float(np.mean([tasks[t]["CORE"]["stable_fraction"] for t in tasks])),
            "mpc_stable":float(np.mean([tasks[t]["SHOOTING_MPC"]["stable_fraction"] for t in tasks]))}

def med(records,key): return float(np.median([r[key] for r in records]))

def adjudicate(records,c):
    m={k:med(records,k) for k in ["core_mpc_cost_ratio","core_mpc_postshift_ratio","core_generic_cost_ratio",
                                  "mpc_frozen_ratio","core_oracle_ratio","core_stable","mpc_stable"]}
    cv=c["comparator_validity"]["medians"]
    cv_checks={"mpc_stable":m["mpc_stable"]>=cv["mpc_stable_min"],
               "mpc_frozen":m["mpc_frozen_ratio"]<=cv["mpc_frozen_ratio_max"]}
    csg=c["comparator_validity"]["seed_guard"]
    cv_sg=sum(r["mpc_stable"]>=csg["mpc_stable_min"] and
              r["mpc_frozen_ratio"]<=csg["mpc_frozen_ratio_max"] for r in records)
    comparator_valid=all(cv_checks.values()) and cv_sg>=c["comparator_validity"]["seed_guard_required"]

    p=c["primary_noninferiority"]["medians"]
    checks={"core_stable":m["core_stable"]>=p["core_stable_min"],
    ...
    sg=c["primary_noninferiority"]["seed_guard"]
    ni_sg=sum(r["core_stable"]>=sg["core_stable_min"] and
              ...
              r["core_generic_cost_ratio"]<=sg["core_generic_cost_ratio_max"] for r in records)
    noninf=comparator_valid and all(checks.values()) and ni_sg>=c["primary_noninferiority"]["seed_guard_required"]

    a=c["exclusive_core_advantage"]
    adv_sg=sum(r["core_mpc_cost_ratio"]<=a["seed_guard_ratio_max"] for r in records)
    adv=comparator_valid and m["core_mpc_cost_ratio"]<=a["core_mpc_cost_ratio_max"] and adv_sg>=a["seed_guard_required"]
    return {"medians":m,
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--criteria",required=True); ap.add_argument("--output",required=True)
    args=ap.parse_args(); c=json.loads(Path(args.criteria).read_text())
    ...
    out={"campaign":c["campaign"],"criteria_status":c["status"],
