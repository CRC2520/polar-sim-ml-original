...
from r8_completion.io import atomic_json, atomic_npz, sha256, verify_npz
...
def run_seed(seed,output):
    ...
    for block in range(PROTOCOL["training_blocks"]):
        for within in range(PROTOCOL["episodes_per_block"]):
            ...
            atomic_npz(output/f"train_{episode:03d}.npz",trace)
        fit_data = fit_agent(agent,traces)
        atomic_npz(output/f"fitted_samples_block_{block:02d}.npz",fit_data)
    ...
    save_agent(agent,output/"frozen_agent.npz")
    ...
    for domain in PROTOCOL["domains"]:
        ...
        atomic_npz(output/f"prediction_{domain}.npz",prediction_trace)
        ...
        for variant in VARIANTS:
            ...
            atomic_npz(output/f"test_{domain}_{variant}.npz",trace)
            ...
            if variant == "full" and domain == "id":
                replay,replay_summary = rollout(agent,700,domain,variant,learn=False)
...
def main():
    ...
    if args.phase == "final":
        raise SystemExit("Final execution locked pending completed review and prospective freeze")
    seeds = PROTOCOL["pilot_seeds"][:args.pilot_count]
